...
namespace dc
{
        ...
        */
                ...
                cv::Mat& vx, cv::Mat& vy)
        {
                ...
                // Build pyramids
                std::vector<cv::Mat> pyrd1(nlevels), pyrd2(nlevels);
                tf::Executor executor;
                tf::Taskflow taskflow;
                auto pyrTask1 = taskflow.emplace([&](){ createPyramids(siftImg1, pyrd1, nlevels); });
                auto pyrTask2 = taskflow.emplace([&](){ createPyramids(siftImg2, pyrd2, nlevels); });
                executor.run(taskflow).wait();

                // For each level
                for (int i = nlevels - 1; i >= 0; --i)
                {
                  const int gammaFact = 1 << i;     // gammaFact = 2^i;
                  const int width = pyrd1[i].cols;
                  const int height = pyrd1[i].rows;
                  const int channels = pyrd1[i].channels();
                  const int width2 = pyrd2[i].cols;
                  const int height2 = pyrd2[i].rows;
                  const int pixels = pyrd1[i].total();
                  ...
                  // Set vx vy
                  float* vx_data = (float*)vx.data;
                  float* vy_data = (float*)vy.data;
                  tf::Taskflow pixel_taskflow;
                  pixel_taskflow.parallel_for(0, pixels, 1, [&](const int j) {
                    vx_data[j] = (float)flow.data()[j * 2];
                    vy_data[j] = (float)flow.data()[j * 2 + 1];
                  });
                  executor.run(pixel_taskflow).wait();
                }
        }
