...
// the regularization is a truncated L1 norm: std::min(s|v(x,y)-v(x+1,y)|,d)
...
//#endif
					...
					HistMin=std::min(HistMin,foo);
					HistMax=std::max(HistMax,foo);
					total++;
				}
		}
		...
		for(ptrdiff_t j=0;j<Width;j++)		// index over x
		{
			...
			for(ptrdiff_t k=-pWinSize[1][index];k<=pWinSize[1][index];k++)  // index over y
				for(ptrdiff_t l=-pWinSize[0][index];l<=pWinSize[0][index];l++)  // index over x
				{
					...
					if(!InsideImage(x,y))
						continue;
					int foo=std::min(pDataTerm[index][(k+pWinSize[1][index])*XWinLength+l+pWinSize[0][index]]/HistInterval,static_cast<double>(nBins-1));
	...
	for(size_t i=0;i<nBins;i++)
	{
		...
		{
			DefaultMatchingScore=std::max(i,static_cast<unsigned long>(1))*HistInterval+HistMin; 
			break;
		}
	}
	//DefaultMatchingScore=std::min(100*DefaultMatchingScore,HistMax/10);
	if(IsDisplay)
#ifdef INTMESSAGE
			...
			for(ptrdiff_t k=-pWinSize[1][index];k<=pWinSize[1][index];k++)  // index over y
				for(ptrdiff_t l=-pWinSize[0][index];l<=pWinSize[0][index];l++)  // index over x
				{
					...
					if(!InsideImage(x,y))
                        ...
                        pDataTerm[index][_ptr]=std::min(pDataTerm[index][_ptr],DefaultMatchingScore);
				}
		}
...
void BPFlow::UpdateSpatialMessage(int x, int y, int plane, int direction)
{
	...
	for(ptrdiff_t l=1;l<nStates;l++)
		message_org[l]=std::min(message_org[l],message_org[l-1]+s);
	...
	{
		int start=std::max(-wsize,shift-wsize1);
		int end=std::min(wsize,shift+wsize1);
	...
	for(ptrdiff_t l=0;l<nStates1;l++)
		message[l]=std::min(message[l],Min);
	...
	if(plane==0) // from vx plane to vy plane
		...
		for(size_t l=0;l<nStates1;l++)
		{
			...
			for(size_t h=0;h<nStates;h++)
				Min=std::min(Min,message_org[h]+pDataTerm[offset].data()[h*nStates1+l]);
			message[l]=Min;
		}
...
//						belief[l]+=std::min((double)abs(l-pWinSize[plane][offset]+pOffset[plane][offset]-pX[(offset-1)*2+plane]+pWinSize[plane][offset-1]-pOffset[plane][offset+1])*s,d);
//				if(i>0) // vertical energy
//					for(int l=0;l<nStates;l++)
//						belief[l]+=std::min((double)abs(l-pWinSize[plane][offset]+pOffset[plane][offset]-pX[(offset-Width)*2+plane]+pWinSize[plane][offset-Width]-pOffset[plane][offset-Width])*s,d);
//				if(j<Width-1)
//					Add2Message(belief,pSpatialMessage[plane][offset*nNeighbors+1].data(),nStates);
//				if(i<Height-1)
//					Add2Message(belief,pSpatialMessage[plane][offset*nNeighbors+3].data(),nStates);
		...
		for(size_t j=0;j<Width;j++)
		{
			...
			for(size_t k=0;k<2;k++)
			{
				if(j<Width-1)
				{
					...
					//s=m_s;
					//d=m_d;
					energy+=std::min((double)abs(pX[offset*2+k]-pWinSize[k][offset]+pOffset[k][offset]-pX[(offset+1)*2+k]+pWinSize[k][offset+1]-pOffset[k][offset+1])*s,d);
				}
				if(i<Height-1)
				{
					s=Im_s.data()[offset*2+k];
					...
					//s=m_s;
					//d=m_d;
					energy+=std::min((double)abs(pX[offset*2+k]-pWinSize[k][offset]+pOffset[k][offset]-pX[(offset+Width)*2+k]+pWinSize[k][offset+Width]-pOffset[k][offset+Width])*s,d);
				}
			}
			int vx=pX[offset*2];
