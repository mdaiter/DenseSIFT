...
public:
	// basic functions
	template <class T>
	static inline T EnforceRange(const T& x,const int& MaxValue) {return std::min(std::max(x,0),MaxValue-1);};
...
inline void ImageProcessing::BilinearInterpolate(const T1* pImage,int width,int height,int nChannels,double x,double y,T2* result)
{
	...
	yy=y;
	double dx,dy,s;
	dx=std::max(std::min(x-xx,static_cast<double>(1)),static_cast<double>(0));
	dy=std::max(std::min(y-yy,static_cast<double>(1)),static_cast<double>(0));
