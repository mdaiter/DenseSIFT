...
void ImageFeature::imSIFT(const Image<T1> imsrc, Image<T2> &imsift,int cellSize,int stepSize,bool IsBoundaryIncluded,int nBins)
{
	...
	for(int k = 0;k<nBins;k++)
	{
		...
		for(int i = 0;i<nPixels; i++)
		{
			temp = std::max(gradient.pData[i*2]*_cos + gradient.pData[i*2+1]*_sin,static_cast<double>(0));
			if(alpha>1)
				temp = pow(temp,alpha);
	...
	for(int i=0;i<sift_height;i++)
		for(int j =0;j<sift_width;j++)
		{
			...
			for(int k = 0;k<siftdim;k++)
				imsift.pData[offset+k] = (unsigned char)std::min(sift_cell.pData[k]/(mag+0.01)*255,static_cast<double>(255));//(unsigned char) __min(sift_cell.pData[k]/mag*512,255);
		}//*/
}
	...
	for(int h=0;h<nScales;h++)
	{
		...
		for(int i=0;i<sift_height;i++)
			for(int j =0;j<sift_width;j++)
			{
				...
				//memcpy(imsift.pData+offset,sift_cell.pData,sizeof(double)*siftdim);
				for(int k = 0;k<siftdim;k++)
					imsift.pData[offset+k] = (unsigned char)std::min(sift_cell.pData[k]/(mag+0.01)*255,static_cast<double>(255));
		}
	}
}
